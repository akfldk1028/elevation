# Bibliography

## Primary papers and projects

- [3D Synthesis for Architectural Design, WACV 2025](https://openaccess.thecvf.com/content/WACV2025/html/Tsai_3D_Synthesis_for_Architectural_Design_WACV_2025_paper.html)
- [MVPainter: Accurate and Detailed 3D Texture Generation via Multi-View Diffusion with Geometric Control](https://arxiv.org/abs/2505.12635)
- [MVPaint: Synchronized Multi-View Diffusion for Painting Anything 3D](https://github.com/3DTopia/MVPaint)
- [Paint3D: Paint Anything 3D with Lighting-Less Texture Diffusion Models](https://github.com/OpenTexture/Paint3D)
- [TexPainter: Generative Mesh Texturing with Multi-view Consistency](https://arxiv.org/abs/2406.18539)
- [TexGen: Text-Guided 3D Texture Generation with Multi-view Sampling and Resampling](https://arxiv.org/abs/2408.01291)
- [ConTEXTure: Consistent Multiview Images to Texture](https://arxiv.org/abs/2407.10558)
- [MeSS: City Mesh-Guided Outdoor Scene Generation with Cross-View Consistent Diffusion](https://arxiv.org/abs/2508.15169)
- [Pro-DG: Procedural Diffusion Guidance for Architectural Facade Generation](https://arxiv.org/abs/2504.01571)
- [Generating Daylight-driven Architectural Design via Diffusion Models](https://zrealli.github.io/DDADesign/)

## Provider documentation

- [Tripo image-to-model API](https://platform.tripo3d.ai/docs/generation)
- [Tripo upload API](https://platform.tripo3d.ai/docs/upload)
- [Tripo task and result API](https://platform.tripo3d.ai/docs/task)
- [Tripo billing](https://platform.tripo3d.ai/docs/billing)

- [Tencent Hunyuan3D SubmitTextureTo3DJob](https://cloud.tencent.com/document/api/1804/126292)
- [Tencent Hunyuan3D billing](https://cloud.tencent.com/document/product/1804/123461)
- [Tencent Hunyuan3D product overview](https://cloud.tencent.com/document/product/1804/120696)
- [Alibaba image generation and editing models](https://help.aliyun.com/zh/model-studio/image-model)
- [Qwen Image Edit API](https://help.aliyun.com/en/model-studio/qwen-image-edit-api)
- [Tripo multiview-to-model API](https://developers.tripo3d.com/en/docs/generation-multiview-to-model/standard)

## Web rendering

- [Three.js WebGLRenderer](https://threejs.org/docs/pages/WebGLRenderer.html)
- [Three.js render targets](https://threejs.org/manual/en/rendertargets.html)
- [three-mesh-bvh](https://github.com/gkjohnson/three-mesh-bvh)

## Community and implementation risk

- [Hunyuan3D 2.1 Windows installation issue](https://github.com/Tencent-Hunyuan/Hunyuan3D-2.1/issues/118)
- [Hunyuan3D 2.1 texture inpainting performance issue](https://github.com/Tencent-Hunyuan/Hunyuan3D-2.1/issues/139)
- [Hunyuan3D 2.1 existing-mesh texture error reports](https://github.com/Tencent-Hunyuan/Hunyuan3D-2.1/issues/114)
