...
function grammar(confidence: number) {
	return {
...
test("runs the complete creative-020 comparison fixture without network or paid resubmission", { timeout: 900_000 }, async (context) => {
	...
	const grammarFixtures = [grammar(0.99), grammar(0.85)];
	...
	const dependencyFactory = createFacadeAgentDependencyFactory(async (config: any) => {
		...
		const validate = production.validate;
		...
		return {
			...
			validate: async (input: any) => {
				const actual = await validate(input);
				if (input.provider === "gpt-image-2" && input.versionId === "v001") {
					actual.accepted = false;
					actual.codes = ["DETAIL_BOUNDS_EXCEEDED"];
				}
				return actual;
			},
