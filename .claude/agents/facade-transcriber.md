...
- `grammar-schema.json`; `context-summary.json` (segment ids, faces, lengths, heights);
...
1. Read the photograph. Write down, before any JSON: the construction, the module and its
...
3. Write the grammar to the path you were given.
4. Draw it, with cwd at the repository root:
   `node tools/facade-pipeline/cli.mjs draw <candidate> "<grammar path>" <scheme name>`
   `"stage":"drawn"` is a pass. Every `draw` call is one attempt, including one that stops
   before the grammar is read (an entrance too wide for any facet stops at `resolve`, and it
   does not move or shrink; narrowing it is a repair). Three attempts, then stop and report.
