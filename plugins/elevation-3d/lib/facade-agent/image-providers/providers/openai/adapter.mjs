...
import { serializeOpenAIImageEditRequest } from "./request.mjs";
import { selectOpenAIImageResponse } from "./response.mjs";
	...
	return Object.freeze({
		...
		async generate(input) {
			try {
				...
				const selected = selectOpenAIImageResponse(payload, headerRemoteId);
				...
				if (typeof selected.encodedImage !== "string") {
				...
				const decoded = await decodeImage(selected.encodedImage, remoteId);
