...
import { buildFacadeArchitecturalPrompt } from "../../prompt.mjs";
import { serializeGoogleImageEditRequest } from "./request.mjs";
import { selectGoogleImageResponse } from "./response.mjs";
...
function canonicalPrompt(evidenceAuthority, brief, output) {
	...
	return buildFacadeArchitecturalPrompt({ candidateId, briefId, evidenceManifestSha256: evidenceAuthority.manifestSha256 }).prompt;
}
	...
	return Object.freeze({
		...
		async generate(input) {
			try {
				...
				const body = JSON.stringify(serializeGoogleImageEditRequest(request));
				...
				const selected = selectGoogleImageResponse(payload, headerRemoteId);
				const remoteId = selected.remoteId;
				if (!response.ok) {
				...
				if (selected.moderationBlocked) throw failure("PROVIDER_MODERATION_BLOCKED", "Provider blocked the request by policy", { status: response.status, remoteId });
				if (selected.imageCount === 0) throw failure("PROVIDER_IMAGE_MISSING", "Provider response did not contain an image", { status: response.status, remoteId });
				if (selected.imageCount !== 1) throw failure("PROVIDER_IMAGE_COUNT_INVALID", "Provider response must contain exactly one image", { status: response.status, remoteId });
				const decoded = await decodeImage(selected.encodedImage, selected.declaredMimeType, remoteId);
				const stableRemoteId = remoteId ?? `gemini-${sha256(decoded.bytes)}`;
				const usage = sanitized(selected.usage, apiKey);
				const result = {
					...decoded,
					...
					rawMeta: sanitized({
						provider: PROVIDER,
						model: MODEL,
						modelVersion: selected.resolvedModel,
						finishReasons: selected.finishReasons,
