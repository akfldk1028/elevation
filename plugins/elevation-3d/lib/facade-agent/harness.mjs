...
function plainData(value, label) {
	if (!value || typeof value !== "object" || Array.isArray(value)
		...
		throw codedError("FACADE_PRESENTATION_RESULT_INVALID", `${label} must be plain data`);
	}
	if (Object.values(Object.getOwnPropertyDescriptors(value)).some((descriptor) => !("value" in descriptor))) {
		throw codedError("FACADE_PRESENTATION_RESULT_INVALID", `${label} must not contain accessors`);
	}
	return value;
}

function persistedPresentationRef(runDir, value, label) {
	const record = plainData(value, label);
	if (typeof record.path !== "string" || record.path.length === 0 || !HEX_SHA256.test(record.sha256 ?? "")) {
		throw codedError("FACADE_PRESENTATION_RESULT_INVALID", `${label} must include a path and SHA-256`);
	}
	const absolute = isAbsolute(record.path) ? record.path : join(runDir, record.path);
	return { path: relativePath(runDir, absolute, label), sha256: record.sha256 };
}

function persistedPresentationMemory(runDir, result, selectedGlbSha256) {
	const returned = plainData(result, "presentation result");
	const memory = plainData(returned.memory_record, "presentation memory record");
	const render = returned.render === undefined ? null : plainData(returned.render, "presentation render record");
	const selected = memory.selected_glb ?? render?.selected_glb;
	const selectedRecord = plainData(selected, "presentation selected GLB");
	if (selectedRecord.sha256 !== selectedGlbSha256) {
		throw codedError("FACADE_PRESENTATION_RESULT_INVALID", "Presentation result is not bound to the selected GLB");
	}
	const output = {
	...
	if (views !== undefined) {
		...
		output.views = Object.fromEntries(Object.entries(viewRecords).map(([name, view]) => {
			const persisted = persistedPresentationRef(runDir, view, `presentation ${name} view`);
			const viewRecord = plainData(view, `presentation ${name} view`);
			const viewHash = viewRecord.selected_glb_sha256 ?? viewRecord.selectedGlbSha256;
			if (viewHash !== selectedGlbSha256) {
				throw codedError("FACADE_PRESENTATION_RESULT_INVALID", `Presentation ${name} view is not bound to the selected GLB`);
			}
			return [name, { ...persisted, selected_glb_sha256: selectedGlbSha256 }];
		}));
	}
	return output;
}
...
function technicalManifestForPresentation(delivery, technicalDelivery, selectedGlbSha256) {
	if (delivery?.selected_glb_sha256 !== selectedGlbSha256
		|| technicalDelivery?.manifest?.selected_glb?.sha256 !== selectedGlbSha256) {
		throw codedError("FACADE_PRESENTATION_TECHNICAL_BINDING_MISMATCH", "Technical delivery is not bound to the selected GLB");
	}
	const manifest = plainData(delivery.memory_record?.manifest, "technical delivery manifest");
	if (typeof manifest.path !== "string" || !HEX_SHA256.test(manifest.sha256 ?? "")) {
		throw codedError("FACADE_PRESENTATION_TECHNICAL_BINDING_MISMATCH", "Technical delivery manifest is unavailable");
	}
...
async function executePresentationStage({
	...
	try {
		...
		const receipt = await writeReceipt(runDir, ORPHAN_PRESENTATION_RECEIPT_PATH, {
		...
		if (receiptPersisted && !run.presentation_receipt) throw error;
		...
		await persistPublicRun(runDir, run);
		if (error instanceof LifecycleHookError) throw error;
		return { presentation: null, failure, aborted };
	}
}
...
async function verifyPresentationArtifactRef(runDir, ref, label) {
	if (!ref || typeof ref.path !== "string" || !HEX_SHA256.test(ref.sha256 ?? "")) {
		throw codedError("FACADE_AGENT_STATE_UNCERTAIN", `${label} reference is invalid`);
	}
	let bytes;
	try { bytes = await safeRead(runDir, join(runDir, ref.path), label); }
	catch (error) { throw codedError("FACADE_AGENT_STATE_UNCERTAIN", `${label} is unavailable`, error); }
	if (sha256(bytes) !== ref.sha256) throw codedError("FACADE_AGENT_STATE_UNCERTAIN", `${label} hash mismatch`);
	return bytes;
}
...
function exactReturnedAuthority(run, execution) {
	if (!execution || typeof execution.provider !== "string" || execution.provider.length === 0
		|| typeof execution.selected_version !== "string" || execution.selected_version.length === 0
		|| !HEX_SHA256.test(execution.selected_glb_sha256 ?? "") || !HEX_SHA256.test(execution.candidate_sha256 ?? "")) return false;
	const expectedExecution = {
		status: "returned", provider: execution.provider, selected_version: execution.selected_version,
		selected_glb_sha256: execution.selected_glb_sha256, candidate_sha256: execution.candidate_sha256,
	};
	if (stableJson(execution) !== stableJson(expectedExecution)) return false;
	const state = run.providers?.[execution.provider];
	const version = state?.versions?.find((item) => item.id === execution.selected_version);
	const delivery = state?.delivery;
	if (state?.status !== "accepted" || version?.status !== "accepted"
		|| version.artifact?.sha256 !== execution.selected_glb_sha256
		|| delivery?.status !== "succeeded" || delivery.provider !== execution.provider
		|| delivery.selected_glb_sha256 !== execution.selected_glb_sha256
		|| stableJson(version.delivery) !== stableJson(delivery.memory_record)
		|| (run.delivery != null && stableJson(run.delivery) !== stableJson(delivery))) return false;
	const initialAuthority = run.status === "running" && run.final === null;
	const terminalAuthority = terminalPresentationRecoveryCandidate(run)
		&& run.final.selected_provider === execution.provider
		&& run.final.selected_version === execution.selected_version
		&& run.final.selected_glb_sha256 === execution.selected_glb_sha256
		&& run.final.delivery_sha256 === delivery.delivery_sha256;
	if (!initialAuthority && !terminalAuthority) return false;
	try {
		const candidates = Object.entries(run.providers).map(([provider, providerState]) => ({
			provider, accepted: providerState?.status === "accepted", score: providerState?.score?.score,
			cost: providerState?.cost,
			diagnostics: { local_correction_count: providerState?.versions?.some((item) => item.id === "v002") ? 1 : 0 },
		}));
		return selectFacadeRecommendation(candidates).technical_winner === execution.provider;
	} catch { return false; }
}
...
async function discoverOrphanPresentationReceipt(runDir, run) {
	...
	try {
		...
		const preflight = await verifyDurableReceipt(runDir, run.preflight_receipt);
		if (preflight?.schema_version !== "arr.elevation3d.facade-preflight-receipt.v1"
			|| preflight.candidate_sha256 !== execution.candidate_sha256) {
			throw codedError("FACADE_PRESENTATION_RECOVERY_UNSAFE", "Orphan presentation receipt candidate authority is invalid");
		}
		const ref = {
		...
		if (error?.code === "FACADE_PRESENTATION_RECOVERY_UNSAFE") throw error;
		throw codedError("FACADE_PRESENTATION_RECOVERY_UNSAFE", "Orphan presentation receipt is invalid", error);
	}
}

async function verifyPresentationReceipt(runDir, run, allowTerminalRecovery = false, allowOrphanReturned = false) {
	if (!run.presentation_receipt) {
		if (allowOrphanReturned) {
			if (run.presentation_execution?.status === "returned" && exactReturnedAuthority(run, run.presentation_execution)) return;
			throw codedError("FACADE_PRESENTATION_RECOVERY_UNSAFE", "Orphan presentation receipt checkpoint or authority is invalid");
		}
		if (allowTerminalRecovery && terminalPresentationRecoveryCandidate(run) && presentationRecoveryAttemptAllowed(run)) return;
		if (run.presentation_execution?.status === "succeeded" || run.final?.status === "winner") {
			throw codedError("FACADE_AGENT_STATE_UNCERTAIN", "Completed presentation is missing its durable receipt");
		}
		return;
	}
	...
	if (run.final?.status === "winner" && !(allowTerminalRecovery && committedPresentationFinalizeCandidate(run))
		...
		throw codedError("FACADE_AGENT_STATE_UNCERTAIN", "Final winner is not bound to the presentation receipt");
	}
}
...
async function authorizeTerminalPresentationRecovery({ normalized, runDir, run, deps, signal }) {
	const provider = run.final.selected_provider;
	if (!normalized.providers.includes(provider)) throw codedError("FACADE_PRESENTATION_RECOVERY_UNSAFE", "Selected provider is not authorized by the resumed configuration");
	if (!presentationRecoveryAttemptAllowed(run)) throw codedError("FACADE_PRESENTATION_RECOVERY_UNSAFE", "Presentation execution may have crossed the local render boundary");
	const state = await loadProvider(runDir, run, provider);
	if (state.status !== "accepted") throw codedError("FACADE_PRESENTATION_RECOVERY_UNSAFE", "Selected provider is not durably accepted");
	const version = state.versions?.find((item) => item.id === run.final.selected_version);
	if (!version || version.status !== "accepted" || version.artifact?.sha256 !== run.final.selected_glb_sha256) {
		throw codedError("FACADE_PRESENTATION_RECOVERY_UNSAFE", "Selected version is not bound to the terminal winner");
	}
	const artifact = await authorizeGlb(runDir, version.artifact);
	const validationReceiptValue = await verifyDurableReceipt(runDir, version.validation_receipt);
	if (version.validation_execution?.status !== "succeeded"
		|| version.validation_execution.receipt_sha256 !== version.validation_receipt?.receipt_sha256
		|| validationReceiptValue.schema_version !== "arr.elevation3d.facade-validation-receipt.v1"
		|| validationReceiptValue.provider !== provider || validationReceiptValue.version_id !== version.id
		|| validationReceiptValue.artifact_sha256 !== artifact.sha256
		|| validationReceiptValue.grammar_sha256 !== version.grammar_sha256
		|| stableJson(validationReceiptValue.validation) !== stableJson(version.validation)
		|| validationReceiptValue.validation?.accepted !== true) {
		throw codedError("FACADE_PRESENTATION_RECOVERY_UNSAFE", "Accepted validation receipt is not bound to the terminal winner");
	}
	const delivery = state.delivery;
	if (delivery?.status !== "succeeded" || delivery.provider !== provider
		|| delivery.selected_glb_sha256 !== artifact.sha256
		|| delivery.delivery_sha256 !== run.final.delivery_sha256
		|| stableJson(run.delivery) !== stableJson(delivery)
		|| stableJson(version.delivery) !== stableJson(delivery.memory_record)) {
	...
	catch (error) { throw codedError("FACADE_PRESENTATION_RECOVERY_UNSAFE", "Technical delivery manifest is invalid", error); }
	if (manifest?.selected_glb?.sha256 !== artifact.sha256) {
		throw codedError("FACADE_PRESENTATION_RECOVERY_UNSAFE", "Technical delivery manifest is not bound to the selected GLB");
	}
	const preflight = await verifyDurableReceipt(runDir, run.preflight_receipt);
	throwIfAborted(signal);
	...
	if ((candidate?.candidate?.candidate_id ?? candidate?.candidate_id) !== normalized.candidateId
		|| sha256(stableJson(persistent(candidate))) !== preflight.candidate_sha256) {
		throw codedError("FACADE_PRESENTATION_RECOVERY_UNSAFE", "Candidate identity changed after preflight");
	}
...
async function recoverTerminalPresentation({ normalized, runDir, run, deps, signal }) {
	let authorized;
	try { authorized = await authorizeTerminalPresentationRecovery({ normalized, runDir, run, deps, signal }); }
	catch (error) {
		if (error?.code === "FACADE_PRESENTATION_RECOVERY_UNSAFE") throw error;
...
async function initialize(config, deps, signal) {
	...
	if (await exists(runPath)) {
		...
		if (persisted.input_sha256 !== inputSha256 || persisted.run_id !== normalized.runId
			|| persisted.candidate_id !== normalized.candidateId || persisted.brief_id !== normalized.briefId) {
		...
		try {
			const adopted = await discoverOrphanPresentationReceipt(runDir, persisted);
			if (adopted) {
				const verifiedReturned = await readFacadeAgentStatusInternal(runDir, false, true);
				if (stableJson(verifiedReturned) !== stableJson(persisted)) {
					throw codedError("FACADE_PRESENTATION_RECOVERY_UNSAFE", "Orphan presentation returned checkpoint changed during verification");
				}
				persisted = adopted;
				await persistPublicRun(runDir, persisted);
				orphanAdopted = true;
			}
		} catch (error) {
			if (error?.code === "FACADE_PRESENTATION_RECOVERY_UNSAFE") throw error;
			throw codedError("FACADE_PRESENTATION_RECOVERY_UNSAFE", "Orphan presentation receipt could not be adopted", error);
		}
		...
		try { run = await readFacadeAgentStatusInternal(runDir, recoveryCandidate || finalizeCandidate); }
			...
			if (!recoveryCandidate && !orphanAdopted) throw error;
			throw codedError("FACADE_PRESENTATION_RECOVERY_UNSAFE", "Terminal presentation recovery state could not be reauthorized", error);
		}
...
async function executeFacade(config, deps, stopAfterStage = null) {
	...
	for (const name of ["loadCandidate", "buildEvidence", "build", "validate", "renderDelivery", "renderPresentation"]) {
	...
	if (initialized.recoverPresentation) {
		try { return await recoverTerminalPresentation({ ...initialized, deps, signal }); }
		catch (error) {
	...
	try {
		...
		for (const provider of normalized.providers) {
			...
			try {
				...
				technicalDeliveries[provider] = delivery;
				run.delivery = null;
				await persistPublicRun(runDir, run);
			} catch (error) {
				if (error instanceof LifecycleHookError) throw error;
		...
		if (decision?.status === "winner" && selectedArtifacts[decision.provider] && deliveries[decision.provider]?.status === "succeeded") {
			...
			presentationFailure = outcome.failure;
		}
		...
		if (decision?.status === "winner" && selectedArtifacts[decision.provider]) {
			...
			if (delivery?.status === "succeeded" && presentation?.receipt) {
...
async function readFacadeAgentStatusInternal(runDirInput, allowTerminalRecovery = false, allowOrphanReturned = false) {
	...
	await verifyPresentationReceipt(runDir, run, allowTerminalRecovery, allowOrphanReturned);
...
export async function readFacadeAgentStatus(runDirInput) {
	return readFacadeAgentStatusInternal(runDirInput, false);
}
